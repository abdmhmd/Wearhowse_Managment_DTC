...
export interface TransactionHeader {
  ...
  /** Optional back-reference for vouchers generated by a purchase order (RV receipts / TRF allocation transfers). */
...
export class TransactionsRepository {
  async createHeader(client: PoolClient, header: TransactionHeader): Promise<TransactionHeader> {
    const query = `
      INSERT INTO transactions (transaction_no, type, status, department_id, warehouse_id, to_warehouse_id, purchase_order_id, created_by, notes)
  ...
  async findHeaderById(id: number): Promise<TransactionHeader | null> {
    const res = await pool.query(
      `SELECT t.id, t.transaction_no, t.type, t.status, t.transaction_date,
              t.department_id, t.warehouse_id, t.to_warehouse_id,
