 ...
 *
 *   1. warehouse_manager, no department_id in payload    -> warehouse + dept derived
 *   2. warehouse_manager, matching department_id         -> accepted
 *   3. warehouse_manager, spoofed foreign department_id  -> 409 conflict
 *   4. warehouse_manager, payload warehouse ignored      -> assigned warehouse is used (no privilege escalation)
 *   5. warehouse_manager, payload warehouse that does not exist -> still derived safely (no crash)
 *   6. multi-assignment warehouse_manager                -> always the first assigned warehouse (deterministic)
 *   7. warehouse_manager, zero assignments + valid warehouse_id  -> 201 (FALLBACK)
 *   8. warehouse_manager, zero assignments + missing warehouse_id -> 400 (FALLBACK)
 *   9. warehouse_manager, zero assignments + MAIN warehouse       -> 400 (FALLBACK)
 *  10. warehouse_manager, zero assignments + non-existent warehouse -> 400 (FALLBACK)
 *  11. warehouse_manager assigned only to a MAIN warehouse  -> 400 (never an eligible destination)
 *  12. warehouse with no department link                -> 400
 *  13. system_admin, no department_id                    -> department = warehouse.department_id
 *  14. system_admin, spoofed foreign department_id       -> 409 conflict
 *  15. system_admin, MAIN warehouse as destination       -> 400
...
describe('Create request: server-derived warehouse + department enforcement', () => {
  ...
  beforeAll(async () => {
    ...
    wmMulti = await seedRoleUser('warehouse_manager', { warehouse_ids: [whA, whB] });
    wmNoDept = await seedRoleUser('warehouse_manager', { warehouse_ids: [whNoDept] });
    wmMain = await seedRoleUser('warehouse_manager', { warehouse_ids: [whMain] });
  ...
  test('1: warehouse_manager with no department_id -> warehouse + department derived', async () => {
  ...
  test('2: warehouse_manager with matching department_id -> accepted', async () => {
  ...
  test('3: warehouse_manager with spoofed foreign department_id -> 409 conflict', async () => {
  ...
  test('8: warehouse_manager with ZERO assignments + missing warehouse_id -> 400 (clear message)', async () => {
  ...
  test('9: warehouse_manager with ZERO assignments + MAIN warehouse -> 400 (main is never a destination)', async () => {
  ...
  test('10: warehouse_manager with ZERO assignments + non-existent warehouse -> 400 (clear message)', async () => {
  ...
  test('11: warehouse_manager assigned ONLY to a MAIN warehouse -> 400 (has assignments, none eligible)', async () => {
  ...
  test('13: system_admin with no department_id -> department derived from warehouse', async () => {
  ...
  test('14: system_admin with spoofed foreign department_id -> 409 conflict', async () => {
    const res = await createRequest(admin.token, whA, items(), deptB);
    expect(res.status).toBe(409);
    expect(res.body.error.code).toBe('DEPARTMENT_WAREHOUSE_MISMATCH');
  });

  test('15: system_admin targeting the MAIN warehouse as destination -> 400', async () => {
